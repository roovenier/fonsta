...
program
	.option('--nocss', 'Write into css fonts file')
...
program
	...
	.action(function(font) {
		...
		if(font) {
			...
			install(fontNameRaw, fontTypes, flags, false).then(function(result) {
				spinner.stop();
				console.log(result);
			}, function(err) {
				spinner.stop();
			...
			Object.keys(deps).reduce(function(prom, font, fontIndex) {
				return prom.then(function() {
					return new Promise(function(resolve, reject) {
						...
						install(font, deps[font], flags, false).then(function(result) {
							spinner.stop();
							console.log(result);
							resolve();
						}, function(err) {
