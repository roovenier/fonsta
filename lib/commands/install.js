			...
			if(body !== '{}') {
				...
				new Download({mode: '755', extract: true})
					...
					.run(function(err, files) {
							...
							fontTypes.map(function(fontTypeItem) {
								if(item.indexOf(fontNameRaw.replace(/[^\w\s]/gi, '') + '_' + fontTypeItem + '_') > -1) {
									...
									if(!helpers.isFontExists(targetPath)) {
										...
										console.log('Warning: ' + fontName + ' font style ' + fontType + ' is already exists.');
									}
								}
							});
						});
						...
						console.log(fontName + ' was succefuly installed with types: ' + fontTypesRes.join(', '));
					});
			} else {
