...
var ff_generator = require('../ff_generator');
...
module.exports = function(fontNameRaw, fontTypes, isSave) {
	...
	request(helpers.api.show  + fontNameRaw, function (error, response, body) {
		if (!error && response.statusCode == 200) {
			if(body !== '{}') {
				console.log('Font found. Downloading starts...'.grey);
				...
				new Download({mode: '755', extract: true})
					...
					.run(function(err, files) {
						...
						var fontObj = {};
						fontObj[fontNameRaw] = deps.dependencies[fontNameRaw] || [];
						deps.dependencies = objectAssign({}, deps.dependencies, fontObj);
							...
							fontTypes.map(function(fontTypeItem) {
								if(item.indexOf(fontNameRaw.replace(/[^\w\s]/gi, '') + '_' + fontTypeItem + '_') > -1) {
									...
									if(!helpers.isFontExists(targetPath)) {
										...
										if(deps.dependencies[fontNameRaw].indexOf(fontTypeItem) === -1) {
											deps.dependencies[fontNameRaw].push(fontTypeItem);
										}
										...
										console.log('Warning: '.yellow + fontName.blue + ' with style '.yellow + fontTypeItem.blue + ' is already exists'.yellow);
									}
								}
							});
						});
						...
						if(fontTypesRes.length > 0) {
							console.log(fontName.blue + ' was installed with styles '.green + fontTypesRes.join(', ').blue);
						}
					});
			} else {
