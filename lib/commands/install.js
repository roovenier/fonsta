				...
				new Download({mode: '644', extract: true})
					...
					.run(function(err, files) {
						...
						dirs.map(function(item) {
							fontTypes.map(function(fontTypeItem) {
								if(!isStyleFound && fontTypesRes.length > 0) {
									console.log('Font has been found. Downloading...'.grey);
									isStyleFound = true;
								}
								...
								if(item.indexOf(fontNameRaw.replace(/[^\w\s]/gi, '') + '_' + fontTypeItem + '_') > -1) {
									...
									if(!helpers.isFontExists(targetPath)) {
										...
										isStylesFound = true;
										console.log('Warning: '.yellow + fontName.blue + ' with the style '.yellow + fontTypeItem.blue + ' is already exists'.yellow);
									}
								}
							});
						});
						...
						if(fontTypesRes.length > 0) {
							...
							console.log(fontName.blue + ' font styles '.red + fontTypes.join(', ').blue + ' was not found'.red);
						}
					});
			} else {
